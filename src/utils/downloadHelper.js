...
export class DownloaderHelper extends EventEmitter {
  ...
  constructor(url, destFolder, options = {}) {
    ...
    this.__isClosingFileStream = false
   ...
   */
  stop() {
    const emitBeforeStop = () => {
      this.__setState(this.__states.BEFORESTOP)
  ...
  retry() {
    this.state = this.__states.STARTED
    return this.start()
  }
    ...
    if (!this.__isResumed) {
      ...
      if (fs.existsSync(this.__filePath)) {
        ...
        if (
          ...
          (this.__opts.override.skipSmaller || downloadedSize >= this.__total)
        ) {
          this.emit('skip', {
  ...
  __hasFinished() {
    return (
      this.state !== this.__states.PAUSED &&
  ...
  __closeFileStream() {
    ...
    this.__isClosingFileStream = true
    return new Promise((resolve, reject) => {
  ...
  __onError(resolve, reject) {
    const removeFile = () => {
      return this.__closeFileStream().finally(() => {
    ...
    return err => {
      ...
      if (!this.__opts.retry) {
        return removeFile().finally(() => reject(err))
      }
      return this.__retry()
        .then(() => resolve(true))
        .catch(_err => removeFile().finally(() => reject(_err ? _err : err)))
    }
  }
  ...
  __retry() {
    ...
    if (
      typeof this.__opts.retry !== 'object' ||
    ...
    return new Promise(resolve =>
      setTimeout(
        () => resolve(this.__downloaded > 0 ? this.resume() : this.start()),
        this.__opts.retry.delay
      )
    )
  }
  ...
  __getFileNameFromHeaders(headers, response) {
    ...
    return (this.__opts.fileName ? this.__getFileNameFromOpts(fileName, response) : fileName)
      .split('.')
      .filter(Boolean)
      .join('.') // remove any potential trailing '.' (just to be sure)
  }
